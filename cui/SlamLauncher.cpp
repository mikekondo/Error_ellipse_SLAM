...
void SlamLauncher::run()
{
  ...
  {
    if (odometryOnly)
    { // オドメトリによる地図構築（SLAMより優先）
      if (cnt == 0)
      {
        ipose = scan.pose;
        ipose.calRmat();
        odom_prevScan = scan;
        //DataServer::set_odomPose(scan.pose);
      }
      mapByOdometry(&scan);
    }
    else
...
void SlamLauncher::mapByOdometry(Scan2D *scan)
{
  ...
  Pose2D lastPose = data_save->get_odomPose();
  printf("last pose: tx=%g, ty=%g, th=%g\n", lastPose.tx, lastPose.ty, lastPose.th);
  //近藤追記
  Pose2D odoMotion;
  ...
  pcmap->makeGlobalMap();
  DataServer::set_odomPose(pose);
  Eigen::Matrix3d mcov;
  double dT = 0.1;
  cvc.calMotionCovarianceSimple(pose, dT, mcov);
  Eigen::Matrix3d odomCov;
  CovarianceCalculator::rotateCovariance(lastPose, mcov, odomCov);
  DataServer::set_odomCov(odomCov);
