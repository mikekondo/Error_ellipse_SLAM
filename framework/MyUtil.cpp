...
void MyUtil::calEigen2D(double (*mat)[2], double *vals, double *vec1, double *vec2)
{
  ...
  vec2[0] = m01 / L; // 固有ベクトル
  vec2[1] = -m00 / L;
  /* cout << "固有値_正規" << vals[0] << "::" << vals[1] << endl; */
  /*
  // 検算
...
/* void MyUtil::calEigen2D_ellipse(double (*mat)[2], double *vals, double *vec1, double *vec2)
{
  double a = mat[0][0];
  double b = mat[0][1];
  double c = mat[1][0];
  double d = mat[1][1];
  cout << "誤差楕円共分散" << a << "::" << b << endl;
  double B = sqrt((a + d) * (a + d) - 4 * (a * d - b * c));
  double x1 = ((a + d) + B) / 2;
  double x2 = ((a + d) - B) / 2;
  vals[0] = x1; // 固有値
  ...
  vec2[0] = m01 / L; // 固有ベクトル
  vec2[1] = -m00 / L;
  cout << "誤差楕円_固有値" << vals[0] << "::" << vals[1] << endl;
  /*
  // 検算
  double ax1 = a*vec1[0] + b*vec1[1];
  double ax2 = x1*vec1[0];
